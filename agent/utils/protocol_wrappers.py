...
async def send_text_async_url(
    ...
    *,
    ...
    """Send *text* to the target agent via URL"""
    client = _client_factory.create(minimal_agent_card(url))
    msg = Message(
        kind="message",
        role=Role.user,
        message_id=uuid.uuid4().hex,
...
def extract_text(obj: Task | Message):
    ...
    if isinstance(obj, Task):
        ...
        if obj.status and obj.status.message:
